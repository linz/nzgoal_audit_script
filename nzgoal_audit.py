...
def fetch_all(root_url: str, date_from: dt.date | None = None, date_to: dt.date | None = None) -> list[tuple[int, str, str]]:
    ...
    while url:
        ...
        for item in items:
            ...
            published_at = dt.datetime.strptime(item.get("first_published_at"), "%Y-%m-%dT%H:%M:%S.%fZ").date() if item.get("published_at") else None
            ...
            if date_from is not None or date_to is not None:
                ...
                if date_to is not None and published_at > date_to:
                    continue
            results.append(
                (int(item_id), str(title), str(published_at) if published_at is not None else "")
            )
...
def main():
    ...
    parser.add_argument("--from-date", '-fd', dest="date_from", help="Audit from date (dd/mm/yy)")
    parser.add_argument("--to-date", '-td', dest="date_to", help="Audit to date (dd/mm/yy)")
    parser.add_argument("--tsv-file", "-tsv", required=True, dest="tsv_file", help="TSV file path")
    ...
    print(form_data.get("123991"))

    date_from = dt.datetime.strptime(args.date_from, "%d/%m/%y").date() if args.date_from else None
    date_to = dt.datetime.strptime(args.date_to, "%d/%m/%y").date() if args.date_to else None
    ...
    missing_data = [(i, name, pub) for i, name, pub in all_items if str(i) not in form_data]
    if not missing_data:
        print("(none)")
    else:
